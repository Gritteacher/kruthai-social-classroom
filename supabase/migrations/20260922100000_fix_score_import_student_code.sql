...
begin
  ...
  if p_classroom_id is null or not exists (select 1 from public.classrooms where id = p_classroom_id) then
  ...
  loop
    ...
    if v_assignment_id is null or v_student_id is null or v_status not in ('ungraded', 'scored', 'leave', 'expired', 'no_score') or v_raw_score is null then
      raise exception 'INVALID_SCORE_IMPORT_ENTRY' using errcode = '22023';
    end if;
    ...
    insert into public.score_entries (
      assignment_id, student_id, student_code, score_status, raw_score, raw_max,
      final_score, final_max, source_type, source_id, updated_at
    ) values (
      v_assignment.id, v_student.id, v_student.student_code, v_status, v_raw_score, v_assignment.raw_max,
    ...
    on conflict (assignment_id, student_id) do update set
      student_code = excluded.student_code, score_status = excluded.score_status,
      raw_score = excluded.raw_score, raw_max = excluded.raw_max,
      final_score = excluded.final_score, final_max = excluded.final_max,
