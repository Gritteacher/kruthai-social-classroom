...
-- AI worksheet grading drafts. The API key remains server-side in Netlify.
...
begin
  if new.status = 'reviewed' and old.status is distinct from new.status then
    ...
    where answer_id = new.id and status = 'completed';
  elsif new.status = 'returned' and old.status is distinct from new.status then
    update public.worksheet_ai_reviews
    set status = 'rejected', updated_at = now()
    where answer_id = new.id and status in ('queued', 'processing', 'completed', 'failed');
  end if;
  return new;
end;
$$;
